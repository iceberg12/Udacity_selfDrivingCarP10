...
double ref_v = 100;  // try to stick to velocity 50
size_t x_start = 0;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      // try adding penalty for speed + steer to avoid high speed at turns
      fg[0] += 280*CppAD::pow(vars[delta_start + i] * vars[v_start+i], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // The upper and lower limits of delta are set to -30 and 30 degrees (values in radians).
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.523598;
    vars_upperbound[i] = 0.523598;
  }
